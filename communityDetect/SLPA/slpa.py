    ...
    def perform_slpa(self, ITERATION):
        ...
        for t in range(self.ITERATION):
            ...
            for i in order:  #for each node
                label_list = {}
                neighbor = self.adjacency_list[i]
                for j in neighbor :  #for each neighbor of the listener
                    # select a label to propagate from speaker j to listener i
                    sum_label = sum(self.node_memory[j].itervalues())
                    label_rate = [float(c)/sum_label for c in self.node_memory[j].values()]
                    keys = self.node_memory[j].keys()
